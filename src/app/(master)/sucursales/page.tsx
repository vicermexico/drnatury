...
async function getBranches() {
  ...
  const { data } = await admin
    .from("branches")
    .select("id, name, address, simultaneous_capacity, is_active")
    .is("deleted_at", null)
    .order("name");
  return data ?? [];
}
          ...
          {branches.map((b) => (
            ...
            >
              ...
              <div className="text-right shrink-0 ml-4">
                <p className="text-sm font-medium text-gray-700">{b.simultaneous_capacity}</p>
                <p className="text-[11px] text-gray-400">simultáneas</p>
              </div>
            </Link>
          ))}
        </div>
