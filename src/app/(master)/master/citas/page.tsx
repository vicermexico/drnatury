...
export default async function CitasPage({
  ...
  const [branches, rawAppts] = await Promise.all([
    getBranches(),
    getAppointments(date, branch_id),
  ]);
    ...
    <div className="max-w-3xl space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/master/citas/bloquear"
            className="rounded-xl bg-red-50 border border-red-200 px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-100 transition shrink-0">
            Bloquear
          </Link>
          <Link href="/master/citas/nueva"
      ...
      {appts.length === 0 ? (
        <div className="rounded-2xl bg-white border border-dashed border-gray-300 p-10 text-center">
          <p className="text-gray-500 text-sm mb-2">Sin citas para este dia</p>
          <Link href="/master/citas/nueva" className="text-blue-600 text-sm underline">
            Agendar una cita
          </Link>
        </div>
      ) : (
        <div className="space-y-8">
          {Array.from(byBranch.entries()).map(([, { branchName, items }]) => (
            <section key={branchName} className="space-y-2">
              ...
              {items.map(({ appt, id }) => (
                <Link key={id} href={`/master/citas/${id}`}
