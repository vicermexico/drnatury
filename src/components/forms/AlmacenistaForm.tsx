...
export function AlmacenistaForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await fetch("/api/almacenistas", {
        method: "POST",
      ...
      if (!res.ok) { setError(data.message ?? "Error al crear el almacenista"); return; }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={isPending}
          ...
          {isPending ? "Guardando…" : "Crear almacenista"}
        </button>
      </div>
    </form>
  );
